...
function default.label:make(c)
	if self.makelabelfunctions == nil then
		...
		self.makelabelfunctions = {}
		for key,val in pairs(self.templates) do
			self.makelabelfunctions[key] = template:make(val,self.formatters)
		end
	end
...
local template = Template.new()
for key,val in pairs(default.label.templates) do
	makesortinglabelfunctions[key] = template:make(val,default.label.formatters)
end
function default.sorting.formatters:label(c)
	local func = makesortinglabelfunctions[c.type] or makesortinglabelfunctions[""]
	if func == nil then return nil end
	local year = c.fields["year"]
	c.fields["year"] = nil
	local l = func(c)
	c.fields["year"] = year
	return l
