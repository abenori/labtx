...
local function change_case_check(str,t,expect)
	isequal(LBibTeX.change_case(str,t),expect)
end
...
local function string_split_check(str,func,expect1,expect2)
	local a,b = LBibTeX.string_split(str,func)
	isequal(a,expect1)
	isequal(b,expect2)
end
...
local function text_prefix_check(str,num,expect)
	isequal(LBibTeX.text_prefix(str,num),expect)
end
...
local function text_length_check(str,expect)
	isequal(LBibTeX.text_length(str),expect)
end
