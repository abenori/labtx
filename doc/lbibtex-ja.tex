...
\NewDocumentCommand{\luafunclist}{v}{\subsection{\texttt{#1}}}
...
\end{itemize}
が出力されます．
\item formattersにもtemplatesのような書式が使えます．たとえば上の\luafunc{BibTeX.formatters:author_editor}の例は
\begin{lstlisting}
...
\item \luavar{BibTeX.crossref.reference_key_name}：クロスリファレンスを表すフィールドのキー名です．デフォルト\luastring{crossref}．
\end{itemize}
...
有用そうな関数群やオリジナルの\BibTeX に存在していた関数が，\luavar{LBibTeX}以下に定義されています．
\luafunclist{LBibTeX.text_prefix(str,num)}
...
\end{lstlisting}
はそれぞれ\luastring{aあ}，\luastring{あい}を返します．\footnote{内部コードはUTF-8なので，\luastring{あ}や\luastring{い}は3byteです．}

\luafunclist{LBibTeX.text_length(str)}
strのバイト数を返しますが，コントロールシークエンス等や引数は加算されません．

\luafunclist{LBibTeX.string_split(str,func)}
...
\luafunclist{LBibTeX.change_case(str,format)}
...
\luafunclist{LBibTeX.split_names(names[,seps])}
...
\luafunclist{LBibTeX.get_name_parts(names)}
...
\begin{enumerate}
\item \luastring{[ ,~\t%-]+}に該当するパターンで区切り，配列を生成する．
...
\luafunclist{LBibTeX.forat_name_by_parts(nameparts,format)}
\luavar{format}にて指定された書式に基づき，名前の整形を行います．
\luavar{nameparts}は\luafunc{LBibTeX.get_name_parts}で得られる戻り値と同じかたちで与えます．
...
\luafunclist{LBibTeX.format_name(name,format)}
...
\luafunclist{LBibTeX.make_name_list(namearray, format, separray[, etalstr])}
複数人の名前の配列から文字列を作ります．
...
ただし，\luavar{namearray}の各項は\luavar{format}に従い整形され（書式は\luafunc{LBibTeX.format_name_by_parts}と同様），またもし\luavar{namearray[n]}が\luastring{others}の場合は，\luavar{namearray[n]}は\luavar{etalstr}に置き換えられます．
デフォルトでは\luavar{etalstr}は空文字列です．

\luafunclist{LBibTeX.remove_TeX_cs(s)}
\luavar{s}から\TeX のコントロールシークエンスを取り除いた文字列を得ます．

%\luafunclist{LBibTeX.citation_check(citations,required)}
...
%
%\luafunclist{LBibTeX.LBibTeX:output_citation_check(citation_check)}
%（これはLBibTeX以下に直接定義されているわけではなく，LBibTeX.LBibTeX以下であることに注意してください．）
%\luafunc{LBibTeX.citation_check}での出力を受けて警告を表示します．
